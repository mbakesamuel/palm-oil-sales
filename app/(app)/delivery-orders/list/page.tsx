...
export default async function DeliveryOrdersListPage(props: {
  ...
  const data = await listDeliveryOrdersForOperations({
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm opacity-75">
            Filter by DO number, or view all within the current financial month
            or year.
          </p>
        </div>
      ...
      <div className="rounded-lg border border-border bg-card p-4 space-y-3">
        <form className="flex flex-wrap items-end gap-3" action="/delivery-orders/list">
          <div className="min-w-[220px]">
            <div className="text-xs font-medium opacity-70">DO number</div>
            <input
              name="q"
              defaultValue={qRaw ?? ""}
              placeholder="e.g. DO-2026-000014"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.rows.length === 0 ? (
                <tr>
                  <td className="px-3 py-8 text-center opacity-70" colSpan={8}>
                ...
                data.rows.map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-b-0">
                    ...
                    <td className="px-3 py-2 tabular-nums">{r.dateIssuedIso}</td>
            ...
            <tfoot>
              <tr className="border-t border-border bg-foreground/2">
                <td className="px-3 py-2 text-xs font-medium opacity-70" colSpan={5}>
                  Totals ({data.periodLabel})
                </td>
                <td className="px-3 py-2 text-right tabular-nums font-semibold">
                  {data.totals.totalQty ? data.totals.totalQty.toLocaleString(undefined) : ""}
                </td>
                <td className="px-3 py-2 text-right tabular-nums font-semibold">
                  {data.totals.totalAmountXaf}
                </td>
                <td className="px-3 py-2 text-right text-xs opacity-70">
                  {data.totals.count} DOs
                </td>
              </tr>
            </tfoot>
          </table>
