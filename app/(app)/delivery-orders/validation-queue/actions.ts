...
export type ValidationQueueFilters = {
  q?: string | null; // DO no or customer substring
  from?: string | null; // YYYY-MM-DD
  to?: string | null; // YYYY-MM-DD
  reviewed?: "only" | "exclude" | "all";
  salesPointId?: number | null; // optional; only meaningful for org-wide managers
};
...
async function requireManagerActor() {
  ...
  if (session.role !== UserRole.MANAGER) throw new Error("Only managers can use this screen.");
...
export async function listPendingDeliveryOrdersForValidation(input?: {
  ...
  const { prisma, session, actor } = await requireManagerActor();
  ...
  // Counters (fast enough; all are indexed by status/dateIssued; reviewedAt also indexed).
  const [totalPending, totalReviewedPending, totalUnreviewedPending] = await Promise.all([
    prisma.deliveryOrder.count({ where: { ...scopeWhere, status: ValidationStatus.PENDING } }),
    prisma.deliveryOrder.count({
      where: { ...scopeWhere, status: ValidationStatus.PENDING, reviewedAt: { not: null } },
    }),
    prisma.deliveryOrder.count({
      where: { ...scopeWhere, status: ValidationStatus.PENDING, reviewedAt: null },
    }),
  ]);
  ...
  // Enforce row-level access (sales point + commercial line). We filter here since this endpoint is used
  // directly by the UI; this prevents leaking row presence via search/filter combos.
...
export async function markDeliveryOrdersReviewed(input: {
  ids: number[];
  ...
  try {
    ...
    const existing = await prisma.deliveryOrder.findMany({
      where: { id: { in: ids } },
      select: { id: true, status: true, salesPointId: true, commercialServiceId: true },
    });
    ...
    revalidatePath("/delivery-orders/validation-queue");
...
export async function validateReviewedDeliveryOrders(input: {
  ids: number[];
}): Promise<
...
> {
  try {
    ...
    const ids = (input.ids ?? []).filter((n) => Number.isFinite(n));
    if (ids.length === 0) return { ok: true, validated: 0, skipped: 0, errors: [] };
