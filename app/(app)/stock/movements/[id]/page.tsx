    ...
    <div className="space-y-6">
      ...
      <article className="text-foreground bg-background max-w-3xl mx-auto print:max-w-none print:mx-0 print:text-black print:bg-white">
            ...
            <tbody>
              {data.lines.map((line) => (
                <tr key={line.id} className="border-b border-border print:border-black/10">
                  <td className="py-2">{line.productLabel}</td>
                  <td className="py-2 text-right tabular-nums">
                    {line.voucherQty}
                  </td>
                  <td className="py-2 text-right tabular-nums">
                    {line.actualQty ?? line.postedQty ?? "-"}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <section className="mt-6 grid gap-2 text-sm">
          ...
          <div>Bota confirmed by: {data.receiverValidatedByName ?? "-"}</div>
          <div>
            Bota confirmation time: {formatDateTime(data.receiverValidatedAtIso)}
          </div>
        </section>
