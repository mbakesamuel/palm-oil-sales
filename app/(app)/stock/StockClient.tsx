...
export function StockClient(props: {
  ...
  receiveTransferAction: (id: string) => Promise<StockGenericResult>;
  ...
  const [tab, setTab] = React.useState<TabId>("on-hand");
...
function OnHandTab(props: {
  ...
  const filtered = React.useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q && !r.productName.toLowerCase().includes(q) && !r.salesPointName.toLowerCase().includes(q)) {
        return false;
      }
      return true;
    });
        ...
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {filtered.map((r) => (
                <tr
                  key={`${r.salesPointId}-${r.productId}`}
...
function MovementsTab(props: {
  ...
  const filtered = React.useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q) {
        const blob = `${r.productName} ${r.salesPointName} ${r.documentNo ?? ""} ${r.userName}`.toLowerCase();
        if (!blob.includes(q)) return false;
      }
      return true;
    });
  }, [rows, salesPointId, kind, search]);
...
type LineDraftRecord = { productId: string } & Record<string, string>;
type LineDraft = LineDraftRecord & { qty: string };
type AdjustmentLineDraft = LineDraftRecord & { deltaQty: string };
...
function ReceiptsTab(props: {
  ...
  const { rows, salesPoints, products, scopedSalesPointId } = props;
  ...
  const [lines, setLines] = React.useState<LineDraft[]>([{ productId: "", qty: "" }]);
  ...
  function resetForm() {
    setEditingId(null);
    setSalesPointId(scopedSalesPointId != null ? String(scopedSalesPointId) : "");
    setSupplierLabel("");
...
function TransfersTab(props: {
  ...
  receiveAction: (id: string) => Promise<StockGenericResult>;
  cancelAction: (id: string) => Promise<StockGenericResult>;
  ...
  function resetForm() {
    setEditingId(null);
    setFromSalesPointId(scopedSalesPointId != null ? String(scopedSalesPointId) : "");
    setToSalesPointId("");
  ...
  async function onReceive(id: string) {
    setBusy(true);
    try {
      const res = await props.receiveAction(id);
      if (res.ok) {
    ...
    <section className="space-y-3">
            ...
            <div className={fieldRowClass}>
              ...
              <div className={fieldControlClass}>
                <select
                  ...
                  onChange={(e) => setFromSalesPointId(e.target.value)}
                  required
      ...
      {reviewDetail ? (
        <TransferReviewDialog
          ...
          onReceive={() => void onReceive(reviewDetail.id)}
          onEdit={() => void openEditById(reviewDetail.id)}
          onCancel={() => setPendingCancel(reviewDetail)}
        />
      ) : null}
  ...
  async function onSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      fd.set(
        ...
        JSON.stringify(
          lines
            .filter((l) => l.productId && l.deltaQty)
            .map((l) => ({ productId: l.productId, deltaQty: l.deltaQty })),
        ),
      );
      const res = await props.saveAction(fd);
      if (res.ok) {
      ...
      {open ? (
        ...
        >
          <form onSubmit={(e) => void onSave(e)} className="space-y-2.5">
            ...
            <LineEditor<AdjustmentLineDraft>
              products={products}
              lines={lines}
              onChange={setLines}
              qtyKey="deltaQty"
              qtyLabel="Delta (+ gain / − loss)"
              allowNegativeQty
            />
...
function ReviewLineTable(props: {
  lines: { productName: string; uom: string; qty: string; deltaQty?: string }[];
  qtyHeader?: string;
}) {
