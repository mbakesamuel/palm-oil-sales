...
import { loadBpoOutboundSaleReceipt } from "../../actions";
...
export default async function BpoOutboundSaleReceiptPage(props: {
      ...
      <article className="text-black bg-white max-w-3xl mx-auto print:max-w-none print:mx-0">
        <header className="border-b border-black/20 pb-4 text-center">
         ...
         {/*  <h1 className="text-2xl font-semibold">{data.companyName}</h1>
          ...
          <p className="mt-3 text-xs uppercase tracking-[0.25em] text-black/60">
            Bottled Palm Oil
          </p>
          <h2 className="mt-1 text-xl font-semibold">Sales Receipt</h2> */}
            ...
            <tbody>
              {data.lines.map((line) => (
                <tr key={line.id} className="border rounded-lg border-black/10">
                  ...
                  <td className="py-2 text-right tabular-nums">
                    {moneyLabel(line.lineNet)}
                  </td>
                  <td className="py-2 p-3 text-right tabular-nums">
                    {moneyLabel(line.lineGross)}
                  </td>
                </tr>
              ))}
            </tbody>
