...
import type { StockIssueResult } from "./actions";
...
export function StockIssuesClient(props: {
  bottledProducts: ProductOpt[];
  movements: MovementRow[];
  canPost: boolean;
  botaAvailable: boolean;
  createAction: (formData: FormData) => Promise<StockIssueResult>;
  embedded?: boolean;
  ...
  return (
    <div className={embedded ? "space-y-6" : "space-y-8"}>
      ...
      >
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            ...
            <button
              ...
              onClick={() => setLines((prev) => [...prev, { productId: bottledProducts[0]?.id ?? "", qtyUnits: "0" }])}
            >
              Add line
            </button>
          </div>
        ...
        <button
          disabled={!canPost || !botaAvailable || bottledProducts.length === 0 || busy}
          ...
          {movements.map((m) => (
            <div key={m.voucherNo} className="rounded-lg border border-border p-4">
              ...
              <ul className="mt-2 text-sm space-y-1">
                {m.lines.map((l) => (
                  <li key={`${m.voucherNo}-${l.productLabel}`} className="flex justify-between">
