...
import {
  saveCommercialServiceRole,
  setCommercialServiceRoleActive,
  type LineRoleRow,
} from "./actions";
...
export function LineRoleDefinitions(props: {
  commercialServiceId: string;
  roles: LineRoleRow[];
  selectedRoleId: string;
  onSelectRole: (id: string) => void;
}) {
  const { commercialServiceId, roles, selectedRoleId, onSelectRole } = props;
  const router = useRouter();

  const rolesForLine = React.useMemo(
    () => roles.filter((r) => r.commercialServiceId === commercialServiceId),
    [roles, commercialServiceId],
  );
  ...
  function openAddForm() {
    if (!commercialServiceId) return;
    resetForm();
    setIsFormOpen(true);
  }

  function startEdit(row: LineRoleRow) {
    setEditingId(row.id);
    setCode(row.code);
    setName(row.name);
  ...
  async function onSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      fd.set("commercialServiceId", commercialServiceId);
      if (editingId) fd.set("id", editingId);
      await saveCommercialServiceRole(fd);
      closeForm({ clearBanner: false });
      setBanner({
        type: "ok",
        text: wasEdit ? "Line role updated." : "Line role created.",
      });
      router.refresh();
    } catch (err) {
      setBanner({
        type: "error",
        text: err instanceof Error ? err.message : "Could not save line role.",
      });
    } finally {
      setBusy(false);
    }
  ...
  async function toggleActive(row: LineRoleRow, nextActive: boolean) {
    setBanner(null);
    setBusy(true);
    try {
      ...
      await setCommercialServiceRoleActive(fd);
      if (!nextActive && selectedRoleId === row.id) {
        const fallback = rolesForLine.find((r) => r.id !== row.id && r.isActive);
        onSelectRole(fallback?.id ?? "");
      }
      setBanner({
        type: "ok",
        text: nextActive ? "Line role reactivated." : "Line role deactivated.",
      });
      router.refresh();
    } catch (err) {
      setBanner({
        type: "error",
        text: err instanceof Error ? err.message : "Could not update line role.",
      });
    } finally {
      setBusy(false);
    }
    ...
    <section className="space-y-4">
      ...
      {isFormOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={editingId ? "Edit line role" : "Add line role"}
          onKeyDown={(e) => {
          ...
          <div className="relative z-10 w-full max-w-md rounded-lg border border-border bg-background text-foreground p-3 shadow-lg">
            <div className="flex items-start justify-between gap-3">
              <div className="text-sm font-semibold">
                {editingId ? "Edit line role" : "Add line role"}
              </div>
              <button
                type="button"
            ...
            >
              ...
              <div className={fieldRowClass}>
                <label className={fieldLabelClass} htmlFor="lineRoleCode">
                  Code
                </label>
                <div className={fieldControlClass}>
                  <input
                    ...
                    placeholder="e.g. weighbridge_clerk"
                    required
                    disabled={busy}
                  />
                  <p className={hintClass}>Stable key; used for default permissions.</p>
                </div>
              </div>

              <div className={fieldRowClass}>
                <label className={fieldLabelClass} htmlFor="lineRoleName">
                  Display name
                </label>
                <div className={fieldControlClass}>
                  <input
                    ...
                    placeholder="e.g. Weighbridge clerk"
                    required
                    disabled={busy}
                  />
                </div>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-1 pl-[7.25rem]">
                ...
                >
                  {editingId ? "Save changes" : "Add line role"}
                </button>
                <button
                  type="button"
                  disabled={busy}
      ...
      <div className="rounded-lg border border-border p-4 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-sm font-semibold">Line role definitions</h2>
            <p className="text-xs opacity-70 mt-0.5">
              Roles for this commercial line. Select a row to configure route access below.
            </p>
          </div>
          <button
            type="button"
            disabled={!commercialServiceId || busy}
            onClick={openAddForm}
        ...
        {rolesForLine.length === 0 ? (
          <p className="text-sm opacity-75">
            No line roles yet. Add one or save the commercial line under Services to seed
            defaults.
          </p>
        ) : (
