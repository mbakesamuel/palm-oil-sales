...
export default async function PricingReportPage() {
  ...
  const schedules = await prismaRetry(() =>
    prisma.productUnitPriceSchedule.findMany({
      ...
      include: { product: { select: { productName: true, productCatId: true } } },
    }),
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm opacity-80 mt-1 print:hidden">
          Unit prices (ex tax) for the open financial year — use Print report for your browser’s print
          dialog. Scopes to FY {String(openFy.financialYear)} (
